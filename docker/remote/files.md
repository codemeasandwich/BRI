...
- `printBanner()` - Print remote client banner
...
**Exports:**
- `runCrudExamples(db)` - Run CRUD and filtering examples over remote connection
...
**Exports:**
- `runArrayUpdateExamples(db, entities)` - Run array and update examples over remote
...
**Exports:**
- `runTransactionExamples(db)` - Run transaction examples over remote
...
Main entry point that runs all examples over WebSocket connection.
