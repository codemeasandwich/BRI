...
A minimal Bun-linked app that pulls BRI from the parent checkout via **`"bri": "file:.."`** ([`package.json`](package.json)) so **`import bri from 'bri'`** resolves to the repo root. Anything you publish separately should depend on **`bri-db`** or **`npm link bri-db`** instead.
